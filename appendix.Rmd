...
Finally, a graph is created which compares the proportions across different ethnical groups.

```{r}
p <- ggplot() + theme_bw()
p1 <- p + geom_boxplot(data = dfdevices, aes(y = ratio, x = area)) + ylim(0,3)
p2 <- p1 + geom_hline(aes(yintercept = 1, col = "Ideal No.\nDevices"), size = 1, lty = "dashed") 
p3 <- p2 + labs(y = "Number of Smart Devices per Household\nMember of 5+ years old", x = "", title = "Figure 1: Availability of Smart Devices per Household", subtitle = "Based on data from 5,849 households with respondents between 5 and 18 years old. Surveyed\nhouseholds were selected through randomised cluster sampling at national level,\nwhere clusters represented different socio-economic strata.", caption = "Source: INEC Multi-Purpose Household Survey 2019", fill = "")
p4 <- p3 + scale_color_manual(name = "", values = c("Minimum No.\nDevices Required" = "#A3BFA5"))
p4
```
